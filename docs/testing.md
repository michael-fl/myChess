...
| Test methods (`@Test` + `@ParameterizedTest`) | 1 269 executed, parameterized cases counted per invocation — full run 2026-08-18 |
| Currently passing | **all** — full run 2026-08-18: 1 269 run, 0 failures, 0 errors, **0 skipped** |
...
| Execution | `mvn test` (Maven Surefire 3.5.2), 11:18 min for the full run on an M1 Pro |
...
| `king-safety` — **depth-stability check, 2026-08-18** | | | | Thirteen of the nineteen cases pin a *move* (the other six pin an evaluation) and were re-searched at depths 8-12 to ask whether the defect survives deeper search. Raw data: [`test-results/kingsafety-depth-stability-4.4.2.jsonl`](../test-results/kingsafety-depth-stability-4.4.2.jsonl). **Only 2 keep the move at every depth** — `qd2_atMove23` (an attack on its own king's file simply not scored) and `hxg4_atMove21` (a pawn recaptured instead of trading off the attacking queen). Both are misjudgments of a *position*, not missed sequences, which is exactly what survives depth. **Six converge**: wrong up to some depth, then right and staying right — `bxf5`, `kxh2`, `g6_atMove14` from depth 9, `ba4`, `kxh3`, `ne8` from depth 11. Those six are what better search would fix as a side effect. **Five oscillate**, and that is the real finding: `f3_atMove33`, `fxg2_atMove13` and `rd3_atMove35` are right at depth 8, wrong at 9, right again from 10; `g6_atMove38` is wrong at 8, right at 9, wrong at 10; `h3_atMove20` is right at 11 and wrong again at 12. A sound evaluation converges as depth grows rather than flip-flopping — oscillation means two moves are scored close together and which one wins depends on the accident of the depth reached. That **supports** the "evaluation before search" sequencing argued in [roadmap § 12.21](roadmap.md#1221-king-safety--m--3060-elo) rather than undermining it; the roadmap already cites `33.f3` for the 8→9 regression, and this check adds the 9→10 recovery it did not have. Consequence for the tally: the family's evidence for an evaluation *term* is 2 clean cases plus 5 oscillating ones plus the two documented score-pinning cases (`keBKOXd1`, `21...Qa3`) — not all nineteen, and not merely two. The check is **relative to today's search** and must be repeated once LMR/PVS land: a case that converges at depth 11 today may become the new evaluation-limited set when game depth reaches 11. |
...
| `pointless-exchange` | 3 | — | — | Exchanges valued by material alone, not by what they leave behind. Both from `StsDefectTest`, both kept at every depth from 8 to 11, so neither is a reach problem: `AT.098` trades the queens off from +0.84 into **−0.98** where the suite's `Qd7` holds +0.82, and `Offer of Simplification.090` hands the bishop over for the f6 knight at once (+2.12 → +0.22) when Stockfish's own line takes on f6 two moves later — the capture is right, the timing is not. myChess's own score *rises* from +47 to +66 cp across those depths in the second case, so it grows more confident the longer it looks. Backed by the aggregate, and this one survived the check: theme 15, *Avoid Pointless Exchange*, produced only **19 best moves of 73** in the v4.4.2 run — the lowest best-move count of any theme — and the depth-10 run confirms it is evaluation-limited at 18 % of headroom captured. A quiescence search driven by captures plus an evaluation with no term for keeping tension is biased toward exactly these moves. See [`sts-history.md`](sts-history.md). |
| `king-activity` | 2 | — | — | The king as a piece that wants to be *active*, not merely safe — distinct from `king-safety` above, and untouched by the shelved king-safety terms. `StsDefectTest.kingActivity085`: from +2.13 myChess plays the flank pawn `1.g4` instead of walking the king in with `1.Kf1` (+2.65) and the advantage is gone, **0.00**. Worth **1 of 100 points at every depth from 8 to 11** — one of the flattest trajectories in the scan. The case joins both findings of the STS theme table in one position: theme 11 scores 62.6 %, and the two weakest themes of the run are both flank-pawn advancement. It resembles the open `king-safety` cases (`20.h3`, `33.f3`), but that is a resemblance between positions, not shared evidence, and it is **not** tracked with § 12.21: that section is about king *safety* — the king under attack — while this family is about the king as an *active* piece. The STS has no king-safety theme at all, so nothing in the suite argues for or against § 12.21. Theme 11 itself turned out **depth**-limited (24 % of headroom captured at depth 10), so the theme score does not back this family either; the two cases rest on their own flat trajectories. |
...
| **total** | **50** | **11** | **7** | 68 markers across seven test classes. |
