...
| Metric | Value |
|---|---|
| Test classes | **85** `*Test.java` on master + **35** helpers and measurement drivers under the same source root (branch `attack-units` adds `KingAttackCurveTest`, `WeightingFunctionAttackUnitTest` and the helper `KingAttackProbe`). Both figures measured on master 2026-09-02, including `KingSafetyFeaturesTest`, `WeightingFunctionKingLineTest` and `StatisticsTest`, plus the helpers `KingSafetyFeatures` and `KingSafetyFeatureProbe`. The helper figure read **34** before and was the *branch* count written into master's row — `git ls-tree` gives 33 tracked helpers on master against 34 on the branch. It said 32 until 2026-08-30 and was corrected one step too far. Class count re-counted 2026-08-27, helpers 2026-08-30 — `ProbeVsEvalBenchmark`, `MatchStyleAnalysis`, `KingAttackUnits`, `KingAttackTexelData`, `TexelKingAttackTuner` added, see [roadmap § 12.26](roadmap.md)) |
| Test methods (`@Test` + `@ParameterizedTest`) | **1 366** on master, 1 391 on branch `attack-units`, **1 372 on branch `king-line-tuned`** — master gained `KingSafetyFeaturesTest` 29 (26 on 2026-09-01, three more for the ordered shelter rule), `WeightingFunctionKingLineTest` 23 and `StatisticsTest` 6 for the king-line danger term (2026-09-02). `king-line-tuned` carries `WeightingFunctionKingLineTest` at **29** rather than 23 (2026-09-04): four cases pinning that a king on the a- or h-file is counted on three real files, one guard that an interior king keeps its own window, and two replacing `anOffBoardFileContributesNothing`, which asserted the pre-repair behavior and therefore pinned the defect — measured, not derived, since the class runs in under a second. Previously 1 311 / 1 336 — the 1 271 of the full run on 2026-08-27, plus two gate-boundary and five `containsIllegalMove` sentinel tests (2026-08-28), twenty anchor-bracket cases and ten `KingAttackUnits` tests (2026-08-29), two Chess960 king-safety cases and one standard-chess one (2026-08-30). The branch adds `KingAttackCurveTest` 5, `WeightingFunctionAttackUnitTest` 14 and six cases in `KingAttackUnitsTest` (10 → 16) for the `attackersOf` / `ofZone` / `placeboCenter` additions that `tools/king-attack-vs-stockfish.py` runs on; `KingAttackProbe` is the branch's one new helper. Derived rather than re-measured, since a full run costs 17 minutes; the counts for the changed classes *are* measured — `WeightingFunctionTest` 58, `BlunderTest` 63. Parameterized cases counted per invocation |
