...
> **The build plan for the next attempt lives in [`king-safety.md`](king-safety.md).** It carries the forensic state of the shelved branches (surveyed 2026-08-29), why STS is the wrong success metric despite its theme name, the dataset that has to be built first, and the abort conditions. This section stays the priority entry and the attempt log.

**First term shipped, 2026-09-23 — v4.8.0, king-attack units.** The "attacker count on the king ring" and "non-linear in the attacker count" bullets below are implemented and merged: weighted attackers on the 3x3 zone index a fitted, monotone nine-entry penalty table. It is the first king-safety term in this engine to survive a pre-registered gate, and it did so against foreign opposition rather than in self-play — net **−6.3 ± 19.8** against a ≥ −10 guard, with the placebo arm putting the price of merely computing it at **−18.0 ± 14.5**. Neither shield nor open-file bullet is done. See [`king-safety.md`](king-safety.md) § 4.17 and [version history](version-history.md).
...
and [§ 12.23](roadmap.md#1223-repetition-draws-are-invisible-to-the-search--done-2026-08-15--15-elo)'s
...
**And the term ships anyway, 2026-09-23 — on a different measurement, not on a re-reading of that one.** The self-play rule stood; what overturned the outcome is a second, separately pre-registered run against opponents that are not myChess: a three-arm anchor gauntlet (base, placebo, candidate) over five foreign engines, **4261 games** at `tc=40/60`, net **−6.3 ± 19.8** against a gate of ≥ −10. The guard did not fire, so the pre-commitment reads as a merge and the term is **v4.8.0**. At ±19.8 the run could never have confirmed +8.4 ± 8.9 and the pre-registration said so before it started — it rules out a large loss and nothing more, which leaves the term's worth unmeasured. **The placebo arm is the result worth carrying**: computing the term and discarding it costs **−18.0 ± 14.5**, an interval excluding zero, against an effect of +11.7 ± 18.1 — the same shape king-line-v2 showed (+10.9 effect, −10.9 cost). A cost ladder prices the term at 8.41 % of throughput, three quarters of it in the per-square query reached from inside the mobility walk. Gauntlet table, gate table and the three recorded regressions in [`king-safety.md`](king-safety.md) § 4.17.
...
|---|---|---|---|
| 1 | ~~**Absolute re-anchor** — [`tools/run-anchor-bracket.sh 4.4.0`](../tools/run-anchor-bracket.sh), recipe in [myChess-ELO-measurement.md](myChess-ELO-measurement.md)~~ — **done 2026-08-17**: v4.4.1 measures **1928 ± 21** at `tc=40/120` | Every strength number since 4.0.0 is a propagated relative delta; [version-history](version-history.md) puts the accumulated uncertainty at **±40 Elo**. The last six releases added ~+130 Elo with no external measurement. | none — it *calibrates* the rest |
...
| 3 | [**§ 12.21 King safety**](roadmap.md#1221-king-safety--m--3060-elo) (with the § 12.7 mobility-weight retune) — **first term shipped 2026-09-23 as v4.8.0**, the attack-unit term | The largest missing evaluation term, and now the only theme with *isolated* test cases: `qe5_atMove9` and `f3_atMove33` in `BlunderTest`. Prerequisites for a tuned attempt are finally in place. **Remaining, in this order (agreed 2026-09-24):** cut the term's cost on `attack-units-chebyshev`; match that branch against the same branch plus the phase ramp (running since 2026-09-25); **if the ramp fails its decision rule**, match the branch against itself *without* the phase blend (`attack-units-chebyshev-no-blend`, added 2026-09-25) — every match the term has played carried the blend, so its worth has never been measured on its own, and the blend is removed only if no-blend's lower bound is ≥ 0 Elo; Texel-tune `kingAttackFactor` on the winner — only then, since the ramp changes the phases the factor acts in; match the tuned factor against the winner. | M, ≈ 30–60 |
| 3a | **Profiling** — JFR over bench v2, then the hottest methods against the JIT's inlining log | Added 2026-09-24. Never done in this project: the attack-unit term is 6 % of the run time and the other 94 % has never been measured. The second cost ladder found a hot method pushed two bytes over HotSpot's 325-byte inlining limit; there may be more. Goes after king safety so it profiles code that stays, and before the search items so their measurements start from a baseline whose costs are known. | none directly — a 10 % speedup is worth about 7 to 10 Elo at the usual 50–70 Elo per doubling |
