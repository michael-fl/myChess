...
class GameTest {
    ...
    // ---- Chess960 detection on Game construction ----
    //
    // When a 960 FEN is imported and wrapped in a Game built from the
    // *standard* config, the Game must still recognise that it is a 960
    // position — the variant signal is in the imported board (specifically
    // its non-default castlingRookFiles), not in the GameConfig the caller
    // happened to pass in.

    @ParameterizedTest(name = "{0}")
    @ValueSource(strings = {
            "rkbbnrnq/pppppppp/8/8/8/8/PPPPPPPP/RKBBNRNQ w FAfa - 0 1",   // cutechess sample
            "bbqnnrkr/pppppppp/8/8/8/8/PPPPPPPP/BBQNNRKR w HFhf - 0 1",   // Scharnagl ID 0
            "rkrnnqbb/pppppppp/8/8/8/8/PPPPPPPP/RKRNNQBB w CAca - 0 1"    // Scharnagl ID 959
    })
    void chess960Fen_importedIntoGameWithStandardConfig_detectedAs960(String fen) {
        var board = Fen.importFEN(fen);
        var game = new Game(Game.standardConfig(), board);

        assertTrue(game.is960(),
                "After importing a 960 FEN the Game must recognise it as 960, "
                        + "even when constructed with Game.standardConfig(); FEN: " + fen);
    }

    @Test
    void allChess960StartPositions_importedIntoGameWithStandardConfig_detectedAs960() {
        // Bulk version of the parameterised test, covering every Scharnagl
        // position except 518 (which is the standard chess starting
        // position and is correctly NOT a 960 game).
        for (int id = 0; id < Chess960StartPositions.COUNT; id++) {
            if (id == Chess960StartPositions.STANDARD_CHESS_ID) {
                continue;
            }
            ...
            assertTrue(game.is960(),
                    "Scharnagl ID " + id + " must be detected as 960 after import; FEN: " + fen);
        }
    }

    // ---- Game.isChess960Position ----
    //
    // Direct unit tests for the static detector that decides whether a
    // given Board represents a 960 game. Covers the three positive paths
    // documented in the implementation (non-default rook files, custom
    // king file with live castling rights, ambiguity-resolution via
    // isStandardStartPosition) and the standard-chess negative paths.

    @Test
    void isChess960Position_standardChessStartPosition_returnsFalse() {
        var board = Board.createNewGame();

        assertFalse(Game.isChess960Position(board),
    ...
    void isChess960Position_standardChessAfterE2E4_returnsFalse() {
        ...
        assertFalse(Game.isChess960Position(game.getBoard()),
    ...
    void isChess960Position_chess960StartPositions_returnsTrue(String fen) {
        ...
        assertTrue(Game.isChess960Position(board),
    ...
    void isChess960Position_allChess960StartPositions_returnsTrueExceptId518() {
        ...
        for (int id = 0; id < Chess960StartPositions.COUNT; id++) {
            ...
            assertEquals(expected, Game.isChess960Position(board),
                    "Scharnagl ID " + id + " (" + fen + ")");
        }
    }

    // ---- Game.isStandardStartPosition ----
    //
    // Byte-for-byte equality check against Board.createNewGame(). Covers
    // the obvious positive and negative paths, plus a sanity check that
    // every 960 starting position with rooks at the standard a-/h-files
    // is still rejected (because of the rest of the back rank differing).

    @Test
    void isStandardStartPosition_freshStandardGame_returnsTrue() {
        var board = Board.createNewGame();

        assertTrue(Game.isStandardStartPosition(board),
    ...
    void isStandardStartPosition_afterFirstMove_returnsFalse() {
        ...
        assertFalse(Game.isStandardStartPosition(game.getBoard()),
    ...
    void isStandardStartPosition_chess960StartPosition_returnsFalse() {
        ...
        assertFalse(Game.isStandardStartPosition(board),
    ...
    void isStandardStartPosition_allChess960StartPositions_returnsFalseExceptId518() {
        ...
        for (int id = 0; id < Chess960StartPositions.COUNT; id++) {
            ...
            boolean expected = id == Chess960StartPositions.STANDARD_CHESS_ID;
            assertEquals(expected, Game.isStandardStartPosition(board),
                    "Scharnagl ID " + id + " (" + fen + ")");
        }
    }
}
